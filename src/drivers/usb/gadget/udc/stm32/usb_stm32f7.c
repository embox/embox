...
static int usb_stm32f7_reset_hnd(struct lthread *self);
static LTHREAD_DEF(usb_stm32f7_reset_lt, usb_stm32f7_reset_hnd, 200);
...
//	pdev->pData = &hpcd;
	...
	ret = irq_attach(USB_IRQ, usb_stm32f7_usb_irq_handler, 0, NULL,
		"usb stm32f7");
	if (ret != 0) {
...
STATIC_IRQ_ATTACH(USB_IRQ, usb_stm32f7_usb_irq_handler, NULL);
