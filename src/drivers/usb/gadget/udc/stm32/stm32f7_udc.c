...
EMBOX_UNIT_INIT(stm32f7_udc_init);

#define USB_MAX_EP0_SIZE 64U

#define EP0_BUFFER_SIZE       1024
#define STM32F7_UDC_EPS_COUNT 8 /* IN and OUT */

#define STM32F7_UDC_IN_EP_MASK  ((1 << 1) | (1 << 2) | (1 << 3))
#define STM32F7_UDC_OUT_EP_MASK ((1 << 1) | (1 << 2) | (1 << 3))
...
struct stm32f7_udc {
	struct usb_udc udc;
	struct usb_gadget_ep *eps[STM32F7_UDC_EPS_COUNT];
	struct usb_gadget_request *requests[STM32F7_UDC_EPS_COUNT];
	struct ep_status ep_info[STM32F7_UDC_EPS_COUNT];
...
static unsigned int stm32f7_udc_ep_in_idx(uint8_t epnum) {
	return 0x4U | epnum;
}

static unsigned int stm32f7_udc_ep_out_idx(uint8_t epnum) {
	return 0x0U | epnum;
}

static uint8_t stm32f7_udc_ep_type(struct usb_gadget_ep *ep) {
	return ep->desc->bm_attributes & USB_DESC_ENDP_TYPE_MASK;
}

static int stm32f7_udc_start(struct usb_udc *udc) {
	usb_stm32f7_init();

	return 0;
}

static int stm32f7_udc_ep_queue(struct usb_gadget_ep *ep, struct usb_gadget_request *req) {
	struct stm32f7_udc *u = (struct stm32f7_udc *)ep->udc;
...
static void stm32f7_udc_ep_enable(struct usb_gadget_ep *ep) {
	struct stm32f7_udc *u;
	unsigned int idx;
	uint8_t ep_addr;
	uint16_t maxpacket;
	uint8_t type;
	...
	u = (struct stm32f7_udc *)ep->udc;
	ep_addr = ep->desc->b_endpoint_address;
	maxpacket = ep->desc->w_max_packet_size;
	type = stm32f7_udc_ep_type(ep);

	if (ep->dir == USB_DIR_IN) {
		idx = stm32f7_udc_ep_in_idx(ep->nr);
	}
	else {
		idx = stm32f7_udc_ep_out_idx(ep->nr);
	}
...
static struct stm32f7_udc stm32f7_udc = {
    .udc =
        {
            .name = "stm32f7 udc",
            .udc_start = stm32f7_udc_start,
            .ep_queue = stm32f7_udc_ep_queue,
            .ep_enable = stm32f7_udc_ep_enable,

            .in_ep_mask = STM32F7_UDC_IN_EP_MASK,
            .out_ep_mask = STM32F7_UDC_OUT_EP_MASK,
        },
};

static int stm32f7_udc_init(void) {
	usb_gadget_register_udc(&stm32f7_udc.udc);
...
static void stm32f7_ll_set_address(struct usb_control_header *req) {
	uint8_t dev_addr;
...
#if 0
static void stm32f7_ll_set_configuration(struct usb_control_header *req) {
    ...
    usb_gadget_set_config(stm32f7_udc.udc.composite, config);
...
static void stm32f7_ll_get_status(struct usb_control_header *req) {
	uint16_t status = 0;
...
/**
* @brief  stm32f7_ll_handle_standard_request
*         Handle USB device requests
* @param  req: USB request
* @retval status
*/
static void stm32f7_ll_handle_standard_request(struct usb_control_header *req) {
	int ret;

	switch (req->b_request) {
	case USB_REQ_SET_ADDRESS:
		stm32f7_ll_set_address(req);
		break;
	case USB_REQ_SET_CONFIG:
		ret = usb_gadget_setup(stm32f7_udc.udc.composite,
	...
	case USB_REQ_GET_CONFIG:
		//stm32f7_ll_get_configuration(req);
		log_debug("GET_CONFIGURATION");
		break;
	case USB_REQ_GET_STATUS:
		stm32f7_ll_get_status(req);
		break;
	case USB_REQ_SET_FEATURE:
		//stm32f7_ll_set_feature(req);
		log_debug("SET_FEATURE");
		break;
	case USB_REQ_CLEAR_FEATURE:
		//stm32f7_ll_clear_feature(req);
...
void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd) {
	...
	stm32f7_udc.ep0_data_len = ctrl.w_length;

	switch (ctrl.bm_request_type & USB_REQ_TYPE_MASK) {
	case USB_REQ_TYPE_STANDARD:
		stm32f7_ll_handle_standard_request(&ctrl);
		break;
	default:
		ret = usb_gadget_setup(stm32f7_udc.udc.composite,
...
void HAL_PCD_DataInStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum) {
	if (epnum != 0U) {
		unsigned int idx = stm32f7_udc_ep_in_idx(epnum);
		struct usb_gadget_request *req;

		stm32f7_udc.ep_info[idx].is_used = 0;

		req = stm32f7_udc.requests[idx];
		stm32f7_udc.requests[idx] = NULL;

		if (req != NULL && req->complete != NULL) {
			req->actual_len = req->len;
			req->complete(stm32f7_udc.eps[idx], req);
		}
		return;
	}
	...
	if (epnum == 0U) {
		pep = &stm32f7_udc.ep_info[0x4 | epnum];
		...
		if (pep->rem_length > pep->maxpacket) {
			...
			if ((pep->maxpacket == pep->rem_length)
			    ...
			    && (pep->total_length < stm32f7_udc.ep0_data_len)) {
				HAL_PCD_EP_Transmit(hpcd, 0U, NULL, 0U);
				stm32f7_udc.ep0_data_len = 0U;
				/* Prepare endpoint for premature end of transfer */
				...
				stm32f7_udc.ep_info[0x4 | epnum].is_used = 0;
				HAL_PCD_EP_Receive(hpcd, 0U, NULL, 0U);
			}
		}
...
void HAL_PCD_DataOutStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum) {
	...
	if ((epnum != 0U) && (idx < STM32F7_UDC_EPS_COUNT)) {
		req = stm32f7_udc.requests[idx];
		if (req != NULL) {
	...
	if (epnum == 0U) {
		pep = &stm32f7_udc.ep_info[0x00 | epnum];
...
void HAL_PCD_ResetCallback(PCD_HandleTypeDef *hpcd) {
	...
	stm32f7_udc.ep_info[0x00U | 0x00U].maxpacket = USB_MAX_EP0_SIZE;

	/* Open EP0 IN */
	HAL_PCD_EP_Open(hpcd, 0x80U, USB_MAX_EP0_SIZE, EP_TYPE_CTRL); /* EP0_MAX_SIZE */

	stm32f7_udc.ep_info[0x04U | 0x00U].maxpacket = USB_MAX_EP0_SIZE;

	for (i = 0; i < ARRAY_SIZE(stm32f7_udc.ep_info); i++) {
		stm32f7_udc.ep_info[i].is_used = 0;
	}
}
