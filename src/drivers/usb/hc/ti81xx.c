...
static void ti_csr_write(uint16_t *ctrl_reg, uint16_t or_val,
	uint16_t and_val) {
	uint16_t ctrl_val;
...
static void ti_fifo_do_read(uint32_t *fifo, uint16_t *count,
	...
	for (tl = min(fifo_len / sizeof(uint32_t), req->len / sizeof(uint32_t));
		...
		req->buf = sizeof(uint32_t) +
			memcpy(req->buf, &fifo_val, sizeof(uint32_t));
...
static enum usb_request_status ti81xx_endp_status_map(
	enum ti81xx_endp_type type,
...
static struct usb_request * ti81xx_endp_req_unbind(
	struct ti81xx_hcd_hci *hcdhci,
	enum ti81xx_endp_type endp_type, int host_endp_n) {
	struct usb_request *req;
...
static int ti81xx_request(struct usb_request *req) {
	...
	REG8_STORE(&tiusb->m_faddr,
		(uint8_t) endp->dev->bus_idx & TI81_USB_FADDR_MASK);
