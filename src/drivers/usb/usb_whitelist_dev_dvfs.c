...
static void usb_wl_sn_len_got(struct usb_request *req, void *arg) {
	...
	usb_endp_control(req->endp->dev->endpoints[0], usb_wl_sn_got, tstrs,
		...
		USB_DESC_TYPE_STRING << 8 | dev->dev_desc.i_serial_number,
...
static void usb_whitelist_close(struct idesc *desc) {
...
static ssize_t usb_whitelist_read(struct idesc *desc, const struct iovec *iov,
	...
	req_rules =
		min(nbyte / sizeof(struct usb_whitelist_rule), wl_conf->rules_n);
...
static int usb_whitelist_ioctl(struct idesc *desc, int request, void *data) {
...
static struct idesc * usb_whitelist_open(struct inode *node,
	struct idesc *idesc) {
	struct file *file;
