 ...
 */
static int stm32f3_pwm_timer_init(void) {
 ...
 */
static int stm32f3_pwm_init(void) {
 ...
 * is called
 */
static int stm32f3_servo_init(struct servo_dev *) {
	if (stm32f3_pwm_timer_init()) {
		return -1;
	}


	if (stm32f3_pwm_init()) {
		log_error("Failed to initialize stm32f3servo\n");
		return -1;
	}
 ...
 */
static int stm32f3_servo_set(struct servo_dev *, int pos) {
	if (pos < MIN_POS)
		pos = MIN_POS;
...
const static struct servo_ops stm32f3_servo_ops = {
	.init = stm32f3_servo_init,
	.set_pos = stm32f3_servo_set,
};

SERVO_DEV_DEF(&stm32f3_servo_ops, NULL);
