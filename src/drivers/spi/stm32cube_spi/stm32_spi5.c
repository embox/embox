...
static int stm32_spi5_init(void) {
	...
	stm32_spi5.spi_controller = spi_c;
	...
	gpio_setup_mode(CONF_SPI5_PIN_CS_PORT,
		(1 << CONF_SPI5_PIN_CS_NR),
		GPIO_MODE_OUT | GPIO_MODE_OUT_PUSH_PULL | GPIO_MODE_IN_PULL_UP);
		
	/* Chip Select is in inactive state by default. */
	gpio_set(CONF_SPI5_PIN_CS_PORT, (1 << CONF_SPI5_PIN_CS_NR), GPIO_PIN_HIGH);
#endif
#endif
	return 0;
}

SPI_CONTROLLER_DEF(SPI_BUS_NAME, &stm32_spic_ops, &stm32_spi5, SPI_BUS_NUM, stm32_spi_pins);
