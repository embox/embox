...
static int stm32_spi2_init(void) {
	...
	stm32_spi2.spi_controller = spi_c;
	...
	gpio_setup_mode(CONF_SPI2_PIN_CS_PORT,
		(1 << CONF_SPI2_PIN_CS_NR),
		GPIO_MODE_OUT | GPIO_MODE_OUT_PUSH_PULL | GPIO_MODE_IN_PULL_UP);
		
	/* Chip Select is in inactive state by default. */
	gpio_set(CONF_SPI2_PIN_CS_PORT, (1 << CONF_SPI2_PIN_CS_NR), GPIO_PIN_HIGH);
#endif
