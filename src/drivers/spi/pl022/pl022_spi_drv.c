...
#define VG1T_SUP	         OPTION_GET(BOOLEAN,vg1t_sup)
...
/**/
#define SSP_CR_MASK_SSE       (0x1UL << 21)
#define SSP_CR_MASK_MS        (0x1UL << 22)
#define SSP_CR_MASK_FRM       (0x3UL << 16)
#define SSP_CR_FRM_SPI        (0x0UL << 16)
...
static inline int pl022_spi_claim_bus(struct pl022_spi *dev) {
	...
	reg |= SSP_CR_MASK_SSE;
	REG32_STORE(SSP_CR0(dev->base_addr), reg);
#endif /* VG1T_SUP */
//	pl022_spi_flush(dev);
...
static int pl022_spi_setup(struct pl022_spi *dev, bool is_master) {
	...
	if (!is_master) {
		...
		reg &= ~((uint32_t)SSP_CR_MASK_MS);
	}

	REG32_STORE(SSP_CPSR(dev->base_addr), DFLT_PRESCALE);
	/* 8 bits per word, high polarity and default clock rate */
	REG32_STORE(SSP_CR0(dev->base_addr), reg);
