...
#define SOCFPGA_L3_LWB              (SOCFPGA_L3_REGS + 0x20)
#define SOCFPGA_L3_MEMMGR           (SOCFPGA_L3_REGS + 0x8C)
#define SOCFPGA_L3_HPSFPGAREGS      (SOCFPGA_L3_REGS + 0x90)
#define SOCFPGA_L3_OCRAM            (SOCFPGA_L3_REGS + 0x9C)
#define SOCFPGA_L3_SDRAM            (SOCFPGA_L3_REGS + 0xA0)
...
#define SOCFPGA_SYSMGR_HANDOFF3     (SOCFPGA_SYSMGR_REGS + 0x8C)
...
static void bridge_enable_handoff(void) {
	...
	fpga2sdram_apply();
	REG32_STORE(SOCFPGA_SDRAM_CONFIG, 0xFF);
	REG32_STORE(SOCFPGA_RESETMGR_AXI_BRIDGE, 0);
	REG32_STORE(SOCFPGA_L3_REMAP, 0x19);
#endif
	REG32_STORE(SOCFPGA_L3_REMAP, 0x19);
	REG32_STORE(SOCFPGA_L3_OCRAM, 0x1);
	REG32_STORE(SOCFPGA_L3_LWB, 0x1);
	REG32_STORE(SOCFPGA_SYSMGR_HANDOFF3, 1);
	REG32_STORE(SOCFPGA_L3_HPSFPGAREGS, 1);
	REG32_STORE(SOCFPGA_L3_MEMMGR, 1);
	REG32_STORE(SOCFPGA_L3_SDRAM, 1);
	uint32_t handoff = REG32_LOAD(SOCFPGA_SYSMGR_HANDOFF3);
	log_debug("Handoff is %08x\n", handoff);
	REG32_STORE(SOCFPGA_SDRAM_CONFIG, 1);
}
