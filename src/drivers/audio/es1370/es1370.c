...
int es1370_drv_resume(int sub_dev) {
	...
	case DAC1_CHAN:
		...
		break;
	case DAC2_CHAN:
		pause_bit = SCTRL_P2LOOPSEL | SCTRL_P2PAUSE | SCTRL_P2DACSEN |
			SCTRL_P2ENDINC | SCTRL_P2STINC;
		break;
	default: return EINVAL;
	}
...
extern int ak4531_init(uint32_t base_addr, uint32_t status_addr,
	uint32_t poll_addr);
static int es1370_hw_init(uint32_t base_addr) {
	...
	if (ak4531_init(base_addr + ES1370_REG_CODEC, base_addr + ES1370_REG_STATUS,
		base_addr + ES1370_REG_CONTROL)) {
...
static int es1370_init(struct pci_slot_dev *pci_dev) {
	...
	if ((err =
		irq_attach(pci_dev->irq, es1370_interrupt, IF_SHARESUP, &es1370_hw_dev,
				"iac"))) {
...
AUDIO_DEV_DEF("es1370_dac1", (struct audio_dev_ops *)&es1370_dev_ops,
	&es1370_dac1);
AUDIO_DEV_DEF("es1370_dac2", (struct audio_dev_ops *)&es1370_dev_ops,
	&es1370_dac2);
AUDIO_DEV_DEF("es1370_adc1", (struct audio_dev_ops *)&es1370_dev_ops,
	&es1370_adc1);
