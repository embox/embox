...
static void iodev_vc_putc(char ch) {
	tty_putc(&video_tty, ch);
}

static char iodev_vc_getc(void) {
	//return diag_getc();
	// ToDo: interface with console mpx
}

static int iodev_vc_kbhit(void) {
	//return diag_kbhit();
	// ToDo: interface with console mpx
}
