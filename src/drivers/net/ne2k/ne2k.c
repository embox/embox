...
static void ne2k_receive(struct net_device *dev) {
	...
	while (--tries) { /* XXX It's an infinite loop or not? */
		...
		copy_data_from_card(ring_offset, (uint8_t *) &rx_frame,
			sizeof(rx_frame),
			base_addr);
		...
		if ((rx_frame.next != next_frame)
			...
			&& (rx_frame.next !=
			next_frame + 1 - (NESM_STOP_PG - NESM_START_PG_RX))) {
			stat->rx_err++;
		...
		if ((total_length < 60) || (total_length > 1518)) {
			...
			skb =
				get_skb_from_card(total_length,
					ring_offset + sizeof(struct e8390_pkt_hdr),
					dev);
			if (skb) {
				stat->rx_packets++;
...
static irq_return_t ne2k_handler(unsigned int irq_num, void *dev_id) {
	...
	while ((isr = in8(base_addr + EN0_ISR)) != 0) {
		if (isr & ENISR_OVER) {
			...
			out8(E8390_NODMA | E8390_PAGE0 | E8390_START,
				base_addr + E8390_CMD);
			ne2k_receive(dev_id); /* Remove packets right away. */
			out8(ENISR_OVER, base_addr + EN0_ISR);
			out8(E8390_TXCONFIG, base_addr + EN0_TXCR); /* xmit on. */
			if (status) {
				out8(E8390_NODMA | E8390_PAGE0 | E8390_START | E8390_TRANS,
					base_addr + E8390_CMD);                                                         /* resend packet */
			}
