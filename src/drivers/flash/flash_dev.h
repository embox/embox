...
struct flash_dev_drv {
	...
	int (*flash_read) (struct flash_dev *dev, uint32_t base, void *data,
		size_t len);
	int (*flash_copy) (struct flash_dev *dev, uint32_t base_to,
		uint32_t base_from, size_t len);
};
...
static inline int flash_read(struct flash_dev *flashdev, unsigned long offset,
	void *buf, size_t len) {
	return flashdev->drv->flash_read(flashdev, offset, buf, len);
}

static inline int flash_write(struct flash_dev *flashdev, unsigned long offset,
...
static inline int flash_copy(struct flash_dev *dev, uint32_t to, uint32_t from,
	size_t len) {
