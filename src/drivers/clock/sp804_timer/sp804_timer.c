...
#define SP804_CR_MASK           (0b11111111) /* Mask for bits 0-7 */
#define SP804_CR_ONESHOT        (1 << 0) /* OneShot bit; 0 -> wrapping mode, 1 -> one-shot mode */
#define SP804_CR_TIMERSIZE      (1 << 1) /* TimerSize bit; 0 -> 16-bit counter, 1 -> 32-bit counter */
...
#define SP804_CR_TIMERMODE      (1 << 6) /* TimerMode bit; 0 -> free-running mode, 1 -> periodic mode */
...
static int sp804_config(struct time_dev_conf * conf) {
    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) & (~SP804_CR_MASK)); /* Unset bits 0-7, also disables the timer to proceed with new config settings */

    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) | SP804_CR_TIMERPRE); /* Set prescaler rate */
    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) | (~SP804_CR_ONESHOT) | (~SP804_CR_TIMERMODE)); /* Select wrapping, free-running mode */
    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) | SP804_CR_TIMERSIZE); /* Select 32-bit counter mode */
    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) | SP804_CR_INTEN); /* Enable interrupts */
	
    REG32_STORE(SP804_T1_CR, REG32_LOAD(SP804_T1_CR) | SP804_CR_TIMEREN); /* Enable timer */
