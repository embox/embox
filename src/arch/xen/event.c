...
#define SET_BIT(bit, \
		field) __asm__ __volatile__ ("lock btsl %1,%0" : "=m" (field) : "Ir" ( \
		bit) : "memory");
#define CLEAR_BIT(field, \
		bit) __asm__ __volatile__ ("lock btrl %1,%0" : "=m" ((field)) : "Ir" (( \
		bit)) : "memory")
...
static inline unsigned long int xchg(unsigned long int *old,
