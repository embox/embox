...
#define DENTRY_NAME_LEN   OPTION_MODULE_GET(embox__fs__dvfs__core, NUMBER, \
		dentry_name_len)
#define DVFS_MAX_PATH_LEN OPTION_MODULE_GET(embox__fs__dvfs__core, NUMBER, \
		max_path_len)
...
struct inode_operations {
	...
	int (*iterate)(struct inode *next, struct inode *parent,
		struct dir_ctx *ctx);
	int (*rename)(struct inode *node, struct inode *new_parent,
		const char *new_name);
	int (*getxattr)(struct inode *node, const char *name, char *value,
		size_t size);
	int (*setxattr)(struct inode *node, const char *name, const char *value,
...
extern struct super_block * dvfs_alloc_sb(const struct dumb_fs_driver *drv,
	struct file *bdev_file);
...
extern int dvfs_mount(const char *dev, const char *dest, const char *fstype,
	int flags);
