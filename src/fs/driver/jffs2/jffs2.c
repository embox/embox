...
static int jffs2_mount(struct nas *dir_nas) {
	...
	c->inocache_list = sysmalloc(
			sizeof(struct jffs2_inode_cache *) * INOCACHE_HASHSIZE);
	if (!c->inocache_list) {
		return ENOMEM;
	}
	memset(c->inocache_list, 0,
		sizeof(struct jffs2_inode_cache *) * INOCACHE_HASHSIZE);
...
static int jffs2_extend_file(struct _inode *inode,
	...
	D1(printk(
		"Writing new hole frag %#x-%#x between current EOF and new page\n",
...
static struct idesc * jffs2fs_open(struct node *node,
 ...
 * file_operation
 */
static struct idesc * jffs2fs_open(struct node *node, struct file_desc *desc,
	int flags) {
	struct nas *nas;
	struct jffs2_file_info *fi;
	struct jffs2_fs_info *fsi;
...
static struct block_dev * jffs_get_flashdev(struct node *dev_node, int *err) {
	...
	if (NULL ==
		(flash_node = vfs_subtree_lookup(vfs_get_root(), flash_node_name))) {
