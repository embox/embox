	...
	while ((*prev) && (*prev)->nhash <= new->nhash) {
		if ((*prev)->nhash == new->nhash &&
			...
			if (new->version < (*prev)->version) {
				JFFS2_DBG_DENTLIST(
					"Eep! Marking new dirent node is obsolete, old is \"%s\", ino #%u\n",
					(*prev)->name, (*prev)->ino);
				jffs2_mark_node_obsolete(c, new->raw);
				jffs2_free_full_dirent(new);
			} else {
				JFFS2_DBG_DENTLIST(
					"marking old dirent \"%s\", ino #%u bsolete\n",
					(*prev)->name, (*prev)->ino);
				new->next = (*prev)->next;
	...
	if (this->node) {
		...
		if (!this->node->frags) {
			...
			JFFS2_DBG_FRAGTREE2(
				...
				ref_offset(
				this->node->raw), this->node->ofs,
				this->node->ofs+this->node->size);
			...
			JFFS2_DBG_FRAGTREE2(
				...
				ref_offset(
				this->node->raw), this->node->ofs, this->node->ofs+this->node->size,
				this->node->frags);
...
static int jffs2_add_frag_to_fragtree(struct jffs2_sb_info *c,
	...
	if (newfrag->ofs > this->ofs) {
		...
		JFFS2_DBG_FRAGTREE2(
			...
			newfrag, newfrag->ofs, newfrag->ofs+newfrag->size, this, this->ofs,
			this->ofs+this->size);
...
struct jffs2_node_frag * jffs2_lookup_node_frag(struct rb_root *fragtree,
	...
	while (next) {
		...
		JFFS2_DBG_FRAGTREE2(
			...
			frag->ofs, frag->ofs+frag->size, frag, frag->rb.rb_left,
			frag->rb.rb_right);
		if (frag->ofs + frag->size <= offset) {
			JFFS2_DBG_FRAGTREE2(
				"going right from frag %#04x-%#04x, before the region we care about\n",
			...
			JFFS2_DBG_FRAGTREE2(
				"going left from frag %#04x-%#04x, after the region we care about\n",
				frag->ofs, frag->ofs+frag->size);
