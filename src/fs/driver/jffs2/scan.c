...
#define noisy_printk(noise, args ...) do { \
		if (*(noise)) { \
			...
			if (!(*(noise))) { \
				printk( \
					KERN_NOTICE "Further such events for this erase block will not be printed\n"); \
			} \
		} \
} while (0)
...
int jffs2_scan_medium(struct jffs2_sb_info *c)
{
	...
	if (c->nr_erasing_blocks) {
		if (!c->used_size && ((c->nr_free_blocks + empty_blocks + bad_blocks)
			...
			printk(
				KERN_NOTICE "Cowardly refusing to erase blocks on filesystem with no valid JFFS2 nodes\n");
			printk(
				KERN_NOTICE "empty_blocks %d, bad_blocks %d, c->nr_blocks %d\n",
...
static int jffs2_scan_eraseblock(struct jffs2_sb_info *c,
	...
	if (ofs == EMPTY_SCAN_SIZE(c->sector_size)) {
		...
		if (jffs2_cleanmarker_oob(c)) {
			...
			switch (ret) {
			case 0:     return cleanmarkerfound ? BLK_STATE_CLEANMARKER :
					   BLK_STATE_ALLFF;
	...
	while (ofs < jeb->offset + c->sector_size) {
		...
		if (*(uint32_t *)(&buf[ofs - buf_ofs]) == 0xffffffff) {
			...
			while (inbuf_ofs < buf_len) {
				if (*(uint32_t *)(&buf[inbuf_ofs]) != 0xffffffff) {
					printk(
						KERN_WARNING "Empty flash at 0x%08x ends at 0x%08x\n",
						empty_start, ofs);
		...
		if (ofs == jeb->offset &&
			je16_to_cpu(node->magic) == KSAMTIB_CIGAM_2SFFJ) {
			printk(
				KERN_WARNING "Magic bitmask is backwards at offset 0x%08x. Wrong endian filesystem?\n",
				ofs);
			DIRTY_SPACE(4);
			ofs += 4;
			continue;
		...
		if (je16_to_cpu(node->magic) == JFFS2_OLD_MAGIC_BITMASK) {
			...
			printk(
				KERN_WARNING "You cannot use older JFFS2 filesystems with newer kernels\n");
			DIRTY_SPACE(4);
			ofs += 4;
			continue;
		}
		...
		crcnode.nodetype = cpu_to_je16( je16_to_cpu(
		...
		if (ofs + je32_to_cpu(node->totlen) >
			...
			printk(
				KERN_WARNING "Node at 0x%08x with length 0x%08x would run over the end of the erase block\n",
				ofs, je32_to_cpu(node->totlen));
			printk(
				KERN_WARNING "Perhaps the file system was created with the wrong erase size?\n");
			DIRTY_SPACE(4);
			ofs += 4;
			continue;
		}
		...
		case JFFS2_NODETYPE_CLEANMARKER:
			...
			if (je32_to_cpu(node->totlen) != c->cleanmarker_size) {
				printk(
					KERN_NOTICE "CLEANMARKER node found at 0x%08x has totlen 0x%x != normal 0x%x\n",
					ofs, je32_to_cpu(node->totlen), c->cleanmarker_size);
				DIRTY_SPACE(PAD(sizeof(struct jffs2_unknown_node)));
				ofs += PAD(sizeof(struct jffs2_unknown_node));
			} else if (jeb->first_node) {
				printk(
					KERN_NOTICE "CLEANMARKER node found at 0x%08x, not first node in block (0x%08x)\n", ofs,
					jeb->offset);
				DIRTY_SPACE(PAD(sizeof(struct jffs2_unknown_node)));
				ofs += PAD(sizeof(struct jffs2_unknown_node));
			} else {
				...
				if (!marker_ref) {
					printk(
						KERN_NOTICE "Failed to allocate node ref for clean marker\n");
					return -ENOMEM;
				}
		...
		default:
			...
			case JFFS2_FEATURE_ROCOMPAT:
				printk(
						KERN_NOTICE "Read-only compatible feature node (0x%04x) found at offset 0x%08x\n",
			...
			case JFFS2_FEATURE_INCOMPAT:
				printk(
						KERN_NOTICE "Incompatible feature node (0x%04x) found at offset 0x%08x\n",
						je16_to_cpu(node->nodetype), ofs);
				return -EINVAL;
	...
	} else if (!ISDIRTY(c->sector_size -
		(jeb->used_size + jeb->unchecked_size))) {
	...
	struct jffs2_sb_info *c, uint32_t ino)
{
	...
	if (!ic) {
		printk(
			KERN_NOTICE "jffs2_scan_make_inode_cache(): allocation of inode cache failed\n");
		return NULL;
	}
	...
	struct jffs2_raw_inode *ri, uint32_t ofs)
{
	...
	if (!raw) {
		printk(
			KERN_NOTICE "jffs2_scan_inode_node(): allocation of node reference failed\n");
		return -ENOMEM;
	}
	...
	struct jffs2_raw_dirent *rd, uint32_t ofs)
{
	...
	if (crc != je32_to_cpu(rd->node_crc)) {
		printk(
			KERN_NOTICE "jffs2_scan_dirent_node(): Node CRC failed on node at 0x%08x: Read 0x%08x, calculated 0x%08x\n",
	...
	if (!raw) {
		jffs2_free_full_dirent(fd);
		printk(
			KERN_NOTICE "jffs2_scan_dirent_node(): allocation of node reference failed\n");
		return -ENOMEM;
	}
