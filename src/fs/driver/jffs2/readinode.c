 ...
 */
static void jffs2_add_tn_to_tree(struct jffs2_tmp_dnode_info *tn,
	struct rb_root *list)
{
...
static inline int read_dnode(struct jffs2_sb_info *c,
	...
	if (ref_flags(ref) == REF_UNCHECKED) {
		...
		if (unlikely(crc != je32_to_cpu(rd->node_crc))) {
			JFFS2_NOTICE(
				"header CRC failed on node at %#08x: read %#08x, calculated %#08x\n",
		...
		if (rd->compr != JFFS2_COMPR_ZERO && je32_to_cpu(rd->csize)) {
			...
			if (!pointed) {
				...
				err = jffs2_flash_read(c, ref_offset(
						ref) + sizeof(*rd), je32_to_cpu(rd->csize),
						&read, buf);
			...
			if (crc != je32_to_cpu(rd->data_crc)) {
				JFFS2_NOTICE(
					"data CRC failed on node at %#08x: read %#08x, calculated %#08x\n",
	...
	JFFS2_DBG_READINODE("dnode @%08x: ver %u, offset %#04x, dsize %#04x\n",
		ref_offset(ref), je32_to_cpu(rd->version), je32_to_cpu(
		rd->offset), je32_to_cpu(rd->dsize));
...
static int jffs2_do_read_inode_internal(struct jffs2_sb_info *c,
	...
	case S_IFLNK:
		...
		if (f->inocache->state != INO_STATE_CHECKING) {
			 ...
			 * operation. */
			f->target =
				kmalloc(je32_to_cpu(latest_node->csize) + 1, GFP_KERNEL);
			if (!f->target) {
				JFFS2_ERROR(
