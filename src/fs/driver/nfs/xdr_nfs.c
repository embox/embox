...
int xdr_nfs_read_file(struct xdr *xs, char *point) {
	...
	case XDR_DECODE:
		...
		if (xdr_u_int(xs, &reply->status) && (STATUS_OK == reply->status)
			...
			&& xdr_bytes(xs, (char **)&data, &reply->datalen,
			XDR_LAST_UINT32)) {
			return XDR_SUCCESS;
		}
		break;
...
int xdr_nfs_write_file(struct xdr *xs, char *point) {
	...
	case XDR_ENCODE:
		...
		if (xdr_nfs_name_fh(xs,
			req->fh) && unaligned_xdr_u_hyper(xs, &req->offset)
...
int xdr_nfs_get_attr(struct xdr *xs, char *point) {
	...
	return (xdr_u_int(xs, &attr->type) && xdr_u_int(xs, &attr->mode)
		   ...
		   && xdr_u_int(xs,
		   &attr->gid) && unaligned_xdr_u_hyper(xs, &attr->size)
		   && unaligned_xdr_u_hyper(xs,
		   &attr->used) && xdr_u_int(xs, &attr->specdata1)
		   && xdr_u_int(xs,
		   &attr->specdata2) && unaligned_xdr_u_hyper(xs, &attr->fsid)
