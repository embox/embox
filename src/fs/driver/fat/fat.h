...
#define FAT_MAX_SECTOR_SIZE OPTION_MODULE_GET(embox__fs__driver__fat, NUMBER, \
		fat_max_sector_size)
...
extern uint32_t fat_get_volinfo(void *bdev, struct volinfo *volinfo,
...
extern int      fat_create_file(struct fat_file_info *fi, struct dirinfo *di,
	char *name, int mode);
