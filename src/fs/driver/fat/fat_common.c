...
extern int fat_read_sector(struct fat_fs_info *fsi, uint8_t *buffer,
	uint32_t sector);
extern int fat_write_sector(struct fat_fs_info *fsi, uint8_t *buffer,
	uint32_t sector);
...
int fat_create_partition(void *dev, int fat_n) {
	...
	case 32:
		...
		memcpy(lbr.ebpb.ebpb32.system, SYSTEM32,
				sizeof(lbr.ebpb.ebpb32.system));
	...
	for (i = reserved; i < reserved + fat_size; i++) {
		if (0 >
			(err =
			block_dev_write(bdev, (void *) fat_sector_buff,
					sizeof(fat_sector_buff), i * blkpersec))) {
			return err;
		}
	}
...
uint32_t fat_get_ptn_start(void *bdev, uint8_t pnum, uint8_t *pactive,
	...
	if (0 >
		block_dev_read(bdev, (char *) fat_sector_buff, sizeof(struct mbr), 0)) {
		return DFS_ERRMISC;
	}
	/* check if that a lbr */
 ...
 */
uint32_t fat_get_volinfo(void *bdev, struct volinfo *volinfo,
...
uint32_t fat_open_dir(struct fat_fs_info *fsi,
	...
	if (dir_is_root(dirname)) {
		...
		return fat_read_sector(fsi, dirinfo->p_scratch,
				dirinfo->currentsector + dirinfo->currentcluster *
				volinfo->secperclus);
	} else {
		uint8_t tmpfn[12];
		uint8_t *ptr = dirname;
		...
		while (*ptr) {
			...
			if (!memcmp(de.name, tmpfn,
				MSDOS_NAME) && (de.attr & ATTR_DIRECTORY)) {
				if (volinfo->filesystem == FAT32) {
...
uint32_t fat_get_next(struct fat_fs_info *fsi,
	...
	if (dir->currententry >= ent_per_sec) {
		...
		if (dir->currentcluster == 0) {
			...
			if (dir->currentsector >= volinfo->secperclus) {
				...
				dir->currentcluster = fat_get_fat_(fsi, dir->p_scratch,
						&tempint,
						dir->currentcluster);
			}
	...
	if (dirent->name[0] == 0xe5 ||
		(dirent->attr & ATTR_LONG_NAME) == ATTR_LONG_NAME) {
		dirent->name[0] = '\0';
	}
...
int fat_root_dir_record(void *bdev) {
	...
	memset(fat_sector_buff, 0, sizeof(fat_sector_buff));
	memcpy(&(((struct dirent *) fat_sector_buff)[0]), &de,
	...
	*successcount = 0;
	...
	while (remain && result == DFS_OK) {
		...
		sector = fi->volinfo->dataarea +
			...
			div(div(fi->pointer, clastersize).rem,
				fi->volinfo->bytepersec).quot;
		...
		if (div(fi->pointer, fi->volinfo->bytepersec).rem) {
			...
			if (remain >= tempreadsize) {
				memcpy(buffer,
					p_scratch + (fi->volinfo->bytepersec - tempreadsize),
					tempreadsize);
		...
		*successcount += bytesread;
		...
		if (div(fi->pointer - bytesread, clastersize).quot !=
			...
			if (((fi->volinfo->filesystem == FAT12) &&
				...
				fi->cluster = fat_get_fat_(fsi, p_scratch, &bytesread,
						fi->cluster);
			}
		}
	}
	...
	while (remain && result == DFS_OK) {
		...
		if (div(fi->pointer, fi->volinfo->bytepersec).rem) {
			...
			if (remain >= fi->volinfo->bytepersec - tempsize) {
				memcpy(p_scratch + tempsize, buffer,
					fi->volinfo->bytepersec - tempsize);
				if (!result) {
 ...
 */
int fat_create_file(struct fat_file_info *fi, struct dirinfo *di, char *name,
	int mode) {
	uint8_t filename[12];
	struct dirent de;
	struct volinfo *volinfo;
	...
	fi->dirsector = di->currentsector + di->currentcluster *
		volinfo->secperclus;
	fi->diroffset = di->currententry - 1;
	fi->cluster = cluster;
