...
POOL_DEF(tmpfs_fs_pool, struct tmpfs_fs_info,
	OPTION_GET(NUMBER,tmpfs_descriptor_quantity));

/* tmpfs file description pool */
POOL_DEF(tmpfs_file_pool, struct tmpfs_file_info,
...
static struct idesc * tmpfs_open(struct node *node, struct file_desc *file_desc,
	int flags);
...
static struct idesc * tmpfs_open(struct node *node, struct file_desc *desc,
	int flags) {
	return &desc->idesc;
}
