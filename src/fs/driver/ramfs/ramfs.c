...
POOL_DEF(ramfs_fs_pool, struct ramfs_fs_info,
	OPTION_GET(NUMBER,ramfs_descriptor_quantity));

/* ramfs file description pool */
POOL_DEF(ramfs_file_pool, struct ramfs_file_info,
...
static struct idesc * ramfs_open(struct node *node, struct file_desc *file_desc,
	int flags);
...
static struct idesc * ramfs_open(struct node *node, struct file_desc *desc,
	int flags) {
	struct nas *nas;
