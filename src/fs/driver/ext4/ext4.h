...
#define EXT4_DIR_ENTRY_CONTENTS_SIZE(d) (EXT4_MIN_DIR_ENTRY_SIZE + \
	(d)->name_len)
/* size with padding */
...
#define ext4_free_inodes_count(group_desc) \
	__ext4_lo_hi(group_desc->free_inodes_count_lo, \
		group_desc->free_inodes_count_hi)
#define ext4_free_blocks_count(group_desc) \
	__ext4_lo_hi(group_desc->free_blocks_count_lo, \
		group_desc->free_blocks_count_hi)
...
extern struct ext4_group_desc * ext4_get_group_desc(unsigned int bnum,
	struct ext4_fs_info *fsi);
extern uint32_t ext4_setbit(uint32_t *bitmap, uint32_t max_bits,
	unsigned int word);
