...
/* file operations */
static struct idesc * ext3fs_open(struct node *node,
 ...
 * file_operation
 */
static struct idesc * ext3fs_open(struct node *node, struct file_desc *desc,
	int flags) {
...
static size_t ext3fs_write(struct file_desc *desc, void *buff, size_t size) {
	...
	/* TODO recalculate */
	if (!(handle =
		journal_start(fsi->journal, 4 * ext3_trans_blocks(datablocks)))) {
		return -1;
	}
...
static int ext3fs_create(struct node *parent_node, struct node *node) {
	 ...
	 */
	if (!(handle =
		journal_start(fsi->journal, 3 * (ext3_trans_blocks(1) + 2)))) {
		return -1;
	}
...
static int ext3_journal_load(journal_t *jp, struct block_dev *jdev,
	block_t start) {
	ext3_journal_superblock_t *sb;
	ext3_journal_specific_t *spec =
		(ext3_journal_specific_t *)jp->j_fs_specific.data;
	char buf[4096];
...
static int ext3fs_mount(void *dev, void *dir) {
	...
	memcpy(dip, (buf
		+ EXT2_DINODE_SIZE(fsi) *
		ino_to_fsbo(fsi, EXT3_JOURNAL_SUPERBLOCK_INODE)),
	...
	if (0 >
		ext3_journal_load(jp, (struct block_dev *) dev_node->nas->fi->privdata,
