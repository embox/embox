...
int ext3_journal_commit(journal_t *jp) {
	...
	dlist_add_prev(&jp->j_committing_transaction->t_next,
		&jp->j_checkpoint_transactions);
...
int ext3_journal_update(journal_t *jp) {
	...
	return journal_write_block(jp, spec->j_sb_buffer->data, 1,
			jp->j_blk_offset);
}
...
int ext3_journal_trans_freespace(journal_t *jp, size_t nblocks) {
	if (EXT3_JOURNAL_NBLOCKS_NEEDED(jp,
		nblocks) > EXT3_JOURNAL_NBLOCKS_PER_TRANS(jp)) {
		return -1;
	}
...
static int journal_write_desc_blocks(journal_t *jp) {
	...
	tag  =
		(ext3_journal_block_tag_t *) (((ext3_journal_header_t *)desc_b->data) +
		1);
...
static int journal_write_commit_block(journal_t *jp) {
	...
	/* Write it. */
	if (0 >=
		journal_write_block(jp, commit_b->data, 1,
