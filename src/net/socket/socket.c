...
static size_t sendto_sock(struct socket *sock, const void *buf, size_t len, int flags,
	...
	if (res == -EINPROGRESS) {
		...
		res_sleep = event_set_wait(sock->sk->sock_is_ready.set, MAX_WAIT_TIME);
		if (res_sleep == ENOERR) {
			/* was resolved */
			res = 1;
		}
...
ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags,
	...
	if (!res) {
		event_set_wait(sock->sk->sock_is_not_empty.set, SCHED_TIMEOUT_INFINITE);
		res = recvfrom_sock(sock, buf, len, flags, daddr, daddrlen);
	}
	/* } */
	sched_unlock();
...
static ssize_t this_read(struct idx_desc *data, void *buf, size_t nbyte) {
	...
	if (!(data->flags & O_NONBLOCK) && len > 0
			&& NULL == skb_queue_front(sock->sk->sk_receive_queue)) {
