...
static int tcp_close(struct sock *sk) {
	...
	tcp_obj_lock(tcp_sk, TCP_SYNC_STATE);
	{
		...
		case TCP_CLOSEWAIT:
			...
			tcp_set_st(sk, tcp_sk->state == TCP_CLOSEWAIT ? TCP_LASTACK : TCP_FINWAIT_1);
...
static int tcp_connect(struct sock *sk,
	...
	tcp_obj_lock(tcp_sk, TCP_SYNC_STATE);
	{
		...
		case TCP_CLOSED:
			...
			tcp_set_st(sk, TCP_SYN_SENT);
			build_tcp_packet(sizeof magic_opts, 0, sk, skb);
			tcph = tcp_hdr(skb);
			tcph->syn = 1;
...
static int tcp_accept(struct sock *sk, struct sockaddr *addr,
	...
	case TCP_LISTEN:
		...
		assert(io_sync_ready(&tcp_newsk->sk->ios, IO_SYNC_WRITING));
...
static int tcp_sendmsg(struct sock *sk, struct msghdr *msg,
	...
	case TCP_CLOSEWAIT:
		...
		while (len != 0) {
			...
			build_tcp_packet(0, bytes, sk, skb);
