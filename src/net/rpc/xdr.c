...
int xdr_mnt_export(struct xdr *xs, void *exp) {
	size_t s;
	char *point;
	export_dir_t *export;
	export = (export_dir_t *)exp;
	...
	if (xdr_u_int(xs, &export->follow)) {
		if (VALUE_FOLLOWS_YES == export->follow) {
			...
			if (xdr_bytes(xs, (char **)&point,
					&export->dir.len, XDR_LAST_UINT32)) {
				return XDR_SUCCESS;
			}
		}
	}
...
int xdr_mnt_service(struct xdr *xs, void *srvc) {
	size_t s;
	char *point;
	mount_service_t *mnt_srvc;

	mnt_srvc = (mount_service_t *)srvc;
	...
	if (xdr_u_int(xs, &mnt_srvc->status)) {
		if (STATUS_OK == mnt_srvc->status) {
			...
			if (xdr_bytes(xs, (char **)&point,
					&mnt_srvc->fh.len, XDR_LAST_UINT32)) {
...
int xdr_nfs_readdirplus(struct xdr *xs, void *srvc) {
	size_t s;
	char *point;
	mount_service_t *mnt_srvc;
