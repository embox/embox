...
static struct sk_buff * build_packet(struct dgram_buf *buf) {
	...
	while ((skb_iter = skb_queue_pop(&buf->fragments))) {
		...
		if (offset == 0) {
			...
			skb->nh.iph->tot_len =
				htons(buf->len + IP_HEADER_SIZE(ip_hdr(skb_iter)));
...
static struct sk_buff * ip_frag_build(const struct sk_buff *big_skb,
	int frag_offset,
	...
	/* Copy IP content */
	memcpy(frag->mac.raw + len, big_skb->mac.raw + frag_offset,
		frag_size - len);
