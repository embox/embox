...
static int ip6_make(const struct sock *sk,
	...
	const struct in6_addr *src_ip6, *dst_ip6;
	...
	dst_ip6 = to_in6 != NULL ? &to_in6->sin6_addr
			: &in6_sk->dst_in6.sin6_addr;
	/* FIXME use route */
	...
	*data_size = min(*data_size, max_size - hdr_size);
	...
	if (in6_sk != NULL) {
		ip6_build(skb->nh.ip6h, *data_size,
				in6_sk->sk.opt.so_protocol, 64, src_ip6, dst_ip6);
	}
