...
sk_buff_t *arp_create(int type, int ptype, in_addr_t dest_ip,
	...
	sk_buff_t *pack;
	struct arphdr *arp;
	if (NULL == dev || NULL == (pack = alloc_skb(ETH_HEADER_SIZE
			+ ARP_HEADER_SIZE, 0))) {
		return NULL;
	}
	 ...
	 */
	if (dev_hard_header(pack, dev, ptype, (void*) dest_hw, (void*) src_hw,
			pack->len) < 0) {
		kfree_skb(pack);
		return NULL;
	}
...
void arp_send(int type, int ptype, in_addr_t dest_ip, struct net_device *dev,
	...
	pack = arp_create(type, ptype, dest_ip, dev, src_ip, dest_hw, src_hw, th);
	if (pack == NULL) {
		return;
	}
	arp_xmit(pack);
...
static int received_req(sk_buff_t *pack) {
	arphdr_t *arp = pack->nh.arph;
	arp_send(ARPOP_REPLY, ETH_P_ARP, arp->ar_sip, pack->dev, arp->ar_tip,
			pack->mac.ethh->h_source, pack->dev->dev_addr, NULL);
	return 0;
}
