...
void sched_timing_init(struct thread *t) {
	...
	t->schedee.sched_attr.sched_time.running_time = 0;
	t->schedee.sched_attr.sched_time.last_sync = clock();
}

clock_t sched_timing_get(struct thread *t) {
	clock_t running = t->schedee.sched_attr.sched_time.running_time;

	if (sched_active(&t->schedee))
		/* Add the least recent time slice (being used now). */
		running += clock() - t->schedee.sched_attr.sched_time.last_sync;

	return running;
}

static void sched_timing_start(struct thread *t, clock_t cur_time) {
	t->schedee.sched_attr.sched_time.last_sync = cur_time;
}

static void sched_timing_stop(struct thread *t, clock_t cur_time) {
	clock_t spent = cur_time - t->schedee.sched_attr.sched_time.last_sync;

	t->schedee.sched_attr.sched_time.running_time += spent;
	//TODO this is for qt
