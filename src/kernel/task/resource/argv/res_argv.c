...
static int task_argv_exec(const struct task *task, const char *path,
	char *const argv[]) {
	int i;
	...
	for (i = 0; i < task_argv->argc; i++) {
		strncpy(task_argv->argv_buff[i], argv[i],
			sizeof(*task_argv->argv_buff));
...
void task_resource_argv_insert(const struct task *task, const char *arg,
	int index) {
	struct task_argv *task_argv;
	...
	strncpy(task_argv->argv_buff[task_argv->argc], arg,
		sizeof(*task_argv->argv_buff));
	task_argv->argv_buff[task_argv->argc][sizeof(*task_argv->argv_buff) -
	1] = '\0';
