...
struct clnt_ops {
	enum clnt_stat (*call)(struct client *clnt, uint32_t procnum,
		xdrproc_t inproc,
...
extern struct client * clnt_create(const char *host, uint32_t prognum,
	uint32_t versnum,
	const char *prot);
extern struct client * clntudp_create(struct sockaddr_in *addr,
	uint32_t prognum,
	uint32_t versnum, struct timeval wait, int *psock);

extern struct client * clnttcp_create(struct sockaddr_in *addr,
	uint32_t prognum,
	uint32_t versnum, int *psock, unsigned int sendsz, unsigned int recvsz);

extern enum clnt_stat clnt_call(struct client *clnt, uint32_t procnum,
	xdrproc_t inproc,
	char *in, xdrproc_t outproc, char *out, struct timeval wait);
