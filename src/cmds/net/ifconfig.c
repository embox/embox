...
static int ifconfig_setup_iface(struct in_device *iface,
	...
	if (args->with_arp) { /* set flag IFF_NOARP (default 0 means ARP enabled) */
		ret =
			(!args->arp ? netdev_flag_up : netdev_flag_down)(iface->dev,
				IFF_NOARP);
		if (ret != 0) {
			return ret;
		}
	...
	if (args->with_promisc) { /* set flag IFF_PROMISC (default 0) */
		ret =
			(args->promisc ? netdev_flag_up : netdev_flag_down)(iface->dev,
				IFF_PROMISC);
		if (ret != 0) {
			return ret;
		}
	...
	if (args->with_allmulti) { /* set flag IFF_ALLMULTI (default 0) */
		ret =
			(args->allmulti ? netdev_flag_up : netdev_flag_down)(iface->dev,
				IFF_ALLMULTI);
		if (ret != 0) {
			return ret;
		}
	...
	if (args->with_p2p) { /* set flag IFF_POINTOPOINT */
		/* TODO use p2p_addr */
		ret =
			(args->p2p ? netdev_flag_up : netdev_flag_down)(iface->dev,
				IFF_POINTOPOINT);
		if (ret != 0) {
			return ret;
		}
	...
	if (args->with_bcast) { /* set broadcast addr */
		if (args->bcast && args->bcast_addr.s_addr) {
			...
			ret =
				(args->bcast ? netdev_flag_up : netdev_flag_down)(iface->dev,
					IFF_BROADCAST);
			if (ret != 0) {
				return ret;
			}
...
static int ifconfig_print_long_info(struct in_device *iface) {
	...
	if (iface->dev->flags & IFF_DEBUG) {
		printf("DEBUG ");
	}
	if (iface->dev->flags & IFF_LOOPBACK) {
		printf("LOOPBACK ");
	}
	if (iface->dev->flags & IFF_POINTOPOINT) {
		printf("POINTOPOINT ");
	}
	if (iface->dev->flags & IFF_NOTRAILERS) {
		printf("NOTRAILERS ");
	}
	if (iface->dev->flags & IFF_RUNNING) {
		printf("RUNNING ");
	}
	if (iface->dev->flags & IFF_NOARP) {
		printf("NOARP ");
	}
	if (iface->dev->flags & IFF_PROMISC) {
		printf("PROMISC ");
	}
	if (iface->dev->flags & IFF_ALLMULTI) {
		printf("ALLMULTI ");
	}
	if (iface->dev->flags & IFF_MULTICAST) {
		printf("MULTICAST ");
	}
	...
	if (!(iface->dev->flags & IFF_LOOPBACK)) {
		printf("\n\tInterrupt:%d Base address:%p", iface->dev->irq,
			(void *)iface->dev->base_addr);
	}
...
static int ifconfig_show_one_iface(struct in_device *iface,
	char use_short_fmt) {
	int ret;

	ret =
		use_short_fmt ? ifconfig_print_short_hdr() : ifconfig_print_long_hdr();
	if (ret != 0) {
		return ret;
	}
...
static int ifconfig_show_all_iface(char show_disabled, char use_short_fmt) {
	...
	for (iface = inetdev_get_first(); iface != NULL;
		...
		if (!(iface->dev->flags & IFF_UP) && !show_disabled) {
			continue;
		}
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; ++i) {
		if (!strcmp("-h", argv[i]) || !strcmp("--help", argv[i])) {
			...
			args.with_bcast = 1;
			args.bcast = argv[i][0] != '-';
			if (args.bcast && inet_aton(argv[i], &args.bcast_addr)) {
				++i;
			}
		}
