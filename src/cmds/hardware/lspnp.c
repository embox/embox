...
static void show_dev(amba_dev_t *dev, bool show_user) {
	...
	if ((!show_user) || (NULL == dev->show_info)) {
		...
		dev_name = amba_registry_get_dev_name(dev->dev_info.venID,
				dev->dev_info.devID);
		print_table_row(dev->slot, dev->dev_info.venID, dev->dev_info.devID,
			ven_name,
...
static void print_ahbm_pnp_dev(uint32_t slot) {
	...
	if (slot > AHB_MASTERS_QUANTITY) {
		printf(
			"print_ahbm_pnp_dev: Too big arg. The quantity of AHB masters is %d\n",
...
static void print_ahbsl_pnp_dev(uint32_t slot) {
	...
	if (slot > AHB_SLAVES_QUANTITY) {
		printf(
			"print_ahbsl_pnp_dev: Too big arg. The quantity of AHB slaves is %d\n",
			AHB_SLAVES_QUANTITY);
		return;
	}
...
static void print_apb_pnp_dev(uint32_t slot) {
	...
	if (slot > APB_QUANTITY) {
		printf(
			"print_apb_pnp_dev: Too big arg. The quantity of APB devices is %d\n",
			APB_QUANTITY);
		return;
	}
