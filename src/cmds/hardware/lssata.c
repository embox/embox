...
static void dump_port_regs_descr(struct ahci_hba *hba, int port)
{
	...
	{
		uint32_t val =
			*(volatile uint32_t *)(offs + generic_port_regs[i].offset);
		printf("%s (%x): %x\n", generic_port_regs[i].name,
			generic_port_regs[i].offset, val);
		i++;
	}
	printf("\n");
}
...
static void dump_sata_regs_descr(struct ahci_hba *hba)
{
	...
	{
		uint32_t val =
			*(volatile uint32_t *)(hba->base_addr + generic_hba_regs[i].offset);
		printf("%s (%x): %x\n", generic_hba_regs[i].name,
			generic_hba_regs[i].offset, val);
		i++;
	}
	printf("\n");
}
