...
static char pgd_raw[(PGD_COUNT + 1) *
MMU_PGD_SIZE] __attribute__ ((aligned(MMU_PGD_SIZE)));
static char pmd_raw[(PMD_COUNT + 1) *
MMU_PMD_SIZE] __attribute__ ((aligned(MMU_PMD_SIZE)));
static char pte_raw[(PTE_COUNT + 1) *
MMU_PTE_SIZE] __attribute__ ((aligned(MMU_PTE_SIZE)));
...
static char virtual_page_raw[(VIRTUAL_PAGES_COUNT + 1) *
MMU_PAGE_SIZE] __attribute__ ((section(".bss.vmem_pages")));
...
static int vmem_alloc_init(void) {
	...
	virt_page_allocator = page_allocator_init(virtual_page_raw,
			VIRTUAL_PAGES_COUNT * MMU_PAGE_SIZE,
			MMU_PAGE_SIZE);
