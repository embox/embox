...
#define MAIN_BITMASK_LENGTH (allocator->capasity / allocator->page_size) / 32
#define REST_MASK_BIT       (allocator->capasity / allocator->page_size) % 32
...
#endif
	if((allocator->capasity / allocator->page_size) <= start_page) {
		return -1;
	}
