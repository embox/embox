...
int rename(const char *src_name, const char *dst_name) {
	...
	if (!FILE_TYPE(from->flags,
		S_IFDIR) && (to && FILE_TYPE(to->flags, S_IFDIR))) {
	...
	*((char *) dvfs_last_link(dst_parent_path)) = '\0';
	...
	if (dst_parent->d_sb == from->d_sb && dst_parent->d_sb->sb_iops->rename) {
		...
		return dst_parent->d_sb->sb_iops->rename(from->d_inode,
				dst_parent->d_inode, dvfs_last_link(
				dst_name));
	} else {
		...
		if (!from->d_sb->sb_iops->remove ||
			!dst_parent->d_sb->sb_iops->create) {
