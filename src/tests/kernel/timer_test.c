...
TEST_CASE("Testing 2 timer_set, one must occur earlier.") {
	...
	if (timer_set(&timer, TIMER_ONESHOT, 50, test_timer_handler1,
		(void *) &data)) {
		test_fail("failed to install timer");
	}

	if (timer_set(&timer2, TIMER_ONESHOT, 100, test_timer_handler2,
		(void *) &data)) {
		test_fail("failed to install timer");
	}
...
TEST_CASE("Timer could be closed before it's occur") {
	...
	test_assert(0 ==
		timer_init_start_msec(&tmr, TIMER_ONESHOT, 50, test_timer_handler_fail,
		NULL));
	timer_close(&tmr);
	usleep(100 * 1000);
}
...
TEST_CASE("Timer could be started many times") {
	...
	test_assert(0 ==
		timer_init(&tmr, TIMER_ONESHOT, test_timer_handler_fail, NULL));
