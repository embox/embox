...
static void fs_test_write_file(const char *path, int open_flags,
	const char *content) {
	int fd;
	int nwritten;
...
static const char fs_test_wr_file_content[] =
	"This is content for write file, long and original\n";
...
static const char fs_test_wr_file_new_content[] =
	"This is content for new write file, even longer and more original\n";
TEST_CASE("Test create operation on fs") {
	fs_test_write_file(fs_test_wr_file_new, O_WRONLY | O_CREAT | O_EXCL,
		fs_test_wr_file_new_content);
}
