...
TEST_CASE("xattr should be ok for clean file") {
	...
	test_assert_zero(check_xattr_list(TEST_CLEAN_FILE_NM, 0, xattr_nms,
		xattr_vls));
}
...
TEST_CASE("xattr should be replaced") {
	...
	test_assert_zero(check_xattr_list(TEST_FILE2_NM, 0, xattr_nms_old,
		xattr_vls_old));
...
TEST_CASE("xattr entry should be added for file with xattr") {
	...
	test_assert_zero(setxattr(TEST_FILE_ADD_NM, xattr_nm3, xattr_vl3,
		strlen(xattr_vl3),
		XATTR_CREATE));
	test_assert_zero(check_xattr_list(TEST_FILE_ADD_NM, 0, xattr_nms,
		xattr_vls));
}
...
TEST_CASE("xattr entry should be added for clean file") {
	...
	test_assert_zero(check_xattr_list(TEST_FILE_ADD_CLEAN_NM, 0, xattr_nms,
		xattr_vls));
}
