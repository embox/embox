...
BOOL test_mmu_read_asi_reg()
{
	printf ("Registers ASI:\n");
	printf ("0x08:\t%X\n",	get_asi_reg8());
	printf ("0x09:\t%X\n",	get_asi_reg9());
...
//modify by Bondarev Anton for decrease data section
...
#define CTX_TABLE_ADDR	0x40010000
...
BOOL test_mmu()
{
	...
	//add by Bonderav Anton for other memory SDRAM for stack i.e.
	pteval = ((0x41000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV | SRMMU_CACHE);  /*40000000 - 41000000: CRAM */
	srmmu_set_pte(g0+41, pteval);

	pteval = ((0x42000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV | SRMMU_CACHE);  /*40000000 - 41000000: CRAM */
	srmmu_set_pte(g0+42, pteval);

	pteval = ((0x43000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV | SRMMU_CACHE);  /*40000000 - 41000000: CRAM */
	srmmu_set_pte(g0+43, pteval);

	pteval = ((0x60000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV /*| SRMMU_CACHE*/);  /*60000000 - 61000000: CRAM */
	srmmu_set_pte(g0+0x60, pteval);

	pteval = ((0x80000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV /*| SRMMU_CACHE*/);  /*60000000 - 61000000: CRAM */
	srmmu_set_pte(g0+0x80, pteval);

	pteval = ((0x90000000 >> 4) | SRMMU_ET_PTE | SRMMU_PRIV /*| SRMMU_CACHE*/);  /*60000000 - 61000000: CRAM */
	srmmu_set_pte(g0+0x90, pteval);
	...
	*((unsigned long **)&pth_addr) =  pthaddr;
	...
	/* do tests*/
	if ( (*((unsigned long *)0xf0041000)) != 0 || (*((unsigned long *)0xf0041004)) != 0x12345678 )
		{ MMU_RETURN (FALSE); }
	...
	* ((volatile unsigned long *)0xf0041004) = 0x87654321;
	...
	if ( (*((volatile unsigned long *)0xf0041000)) != 0x12345678 || (*((volatile unsigned long *)0xf0041004)) != 0xabcdef01)
	{MMU_RETURN (FALSE); }

	for (j=0xf0043000,i = 3;i<TLBNUM+3;i++,j+=0x1000)
	{
		*((unsigned long *)j) = j;
		if ( *((unsigned long*) (((unsigned long)&page2)+(((i-3)%3)*PAGE_SIZE))) != j )
			{ MMU_RETURN (FALSE); }
	}
	...
	if (!srmmu_pte_young(m0[2])) { MMU_RETURN (FALSE); }
	if (!srmmu_pte_young(p0[2])) { MMU_RETURN (FALSE); }
 	MMU_RETURN (TRUE);
}
