...
*/
static void processTxRxOnce(CanardEmboxInstance *socketcan, int32_t timeout_msec)
{
    // Transmitting
    for (const CanardCANFrame* txf = NULL; (txf = canardPeekTxQueue(&canard)) != NULL;) {
        const int16_t tx_res = canardEmboxTransmit(socketcan, txf, 0);
    ...
    const int16_t rx_res = canardEmboxReceive(socketcan, &rx_frame, timeout_msec);
    if (rx_res < 0) {
...
int main(int argc, char** argv)
{
     ...
     */
    CanardEmboxInstance socketcan;
    const char* const can_iface_name = argv[1];
    int16_t res = canardEmboxInit(&socketcan, can_iface_name);
    if (res < 0) {
