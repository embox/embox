...
int leddrv_ll_error(int n) {
	...
	return !(GPIO_ReadInputData(leddrv_alarms[line].gpio) &
		   leddrv_alarms[line].pin);
}

static inline void leddrv_pin_init(const struct leddrv_pin_desc *pd,
	unsigned int mode) {
...
#if LEDBLOCK_MAJOR == 1
static void leddrv_ll_shift_out(int led_in_line,
	unsigned char leds_state[LEDDRV_LED_N]) {
