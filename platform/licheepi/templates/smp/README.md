...
we cannot use that unfortunately. The reason is that `go` command only works for the BSP, APs still
not boot up.

For u-boot normal case, the APs spinning in the `secondary_hart_loop` and  waiting for `bootx`
cmd, BUT th1520 vendor uboot does some $hity modification. we cannot boot APs even use `bootx`
...
2. Apply the patch using `git am smp-uboot.patch` command under vendor u-boot dir.
