...
int write_raw_little_endian64(unsigned long long value, void *_buffer,
	int offset) {
	offset = write_raw_byte((char)((value) & 0xFF), _buffer, offset);
	offset = write_raw_byte((char)((value >>  8) & 0xFF), _buffer, offset);
	offset = write_raw_byte((char)((value >> 16) & 0xFF), _buffer, offset);
...
int write_raw_varint32(unsigned long value, void *_buffer, int offset) {
	while (1) {
		if ((value & ~0x7F) == 0) {
			...
			offset = write_raw_byte((char)((value & 0x7F) | 0x80), _buffer,
					offset);
			value = value >> 7;
		}
	}
...
int read_raw_little_endian64(unsigned long long *tag, void *_buffer,
	int offset) {
			...
			if (result >= 0) {
				...
				*tag |= ((unsigned long)result) << 28;
				if (result < 0) {
					...
					for (i = 0; i < 5; ++i) {
						offset =
							read_raw_byte((char *)&result, _buffer, offset);
						if (result >= 0) {
							return offset;
						}
					}
...
int Message_can_read_delimited_from(void *_buffer, int offset,
	unsigned int length) {
	unsigned long size;
	int payload_offset;
 ...
 *******************************************************************/
int DeviceType_write_with_tag(enum DeviceType *_DeviceType, void *_buffer,
	int offset, int tag) {
	/* Write tag.*/
...
void DeviceConfigurationMessage_clear(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage) {
	_memset(_DeviceConfigurationMessage, 0,
		sizeof(struct DeviceConfigurationMessage));
}

void DeviceConfigurationMessage_init_optional_attributes(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage) {
}

int DeviceConfigurationMessage_is_default_message(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage) {
...
int DeviceConfigurationMessage_write(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage,
	void *_buffer, int offset) {
	/* Write content of each message element.*/
	offset = write_raw_varint32((1<<3)+2, _buffer, offset);
	offset = write_raw_varint32(_DeviceConfigurationMessage->_id_len, _buffer,
			offset);
	offset = write_raw_bytes(_DeviceConfigurationMessage->_id,
			_DeviceConfigurationMessage->_id_len, _buffer,
			offset);

	offset = DeviceType_write_with_tag(&_DeviceConfigurationMessage->_type,
			_buffer, offset, 2);

	offset = write_raw_varint32((3<<3)+2, _buffer, offset);
	offset = write_raw_varint32(_DeviceConfigurationMessage->_sensors_len,
			_buffer, offset);
	offset = write_raw_bytes(_DeviceConfigurationMessage->_sensors,
			_DeviceConfigurationMessage->_sensors_len, _buffer,
			offset);

	offset = write_raw_varint32((4<<3)+2, _buffer, offset);
	offset = write_raw_varint32(_DeviceConfigurationMessage->_commands_len,
			_buffer, offset);
	offset = write_raw_bytes(_DeviceConfigurationMessage->_commands,
			_DeviceConfigurationMessage->_commands_len,
			_buffer, offset);

	return offset;
}

int DeviceConfigurationMessage_write_with_tag(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage,
	void *_buffer, int offset, int tag) {
	/* Write tag.*/
	offset = write_raw_varint32((tag<<3)+2, _buffer, offset);
	/* Write content.*/
	offset = DeviceConfigurationMessage_write_delimited_to(
			_DeviceConfigurationMessage, _buffer, offset);

	return offset;
}

int DeviceConfigurationMessage_write_delimited_to(
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage,
	void *_buffer, int offset) {
	int i, shift, new_offset, size;

	new_offset = DeviceConfigurationMessage_write(_DeviceConfigurationMessage,
			_buffer, offset);
...
int DeviceConfigurationMessage_read(void *_buffer,
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage, int offset,
	int limit) {
	unsigned int i = 0;
	unsigned long tag = i;
	...
	DeviceConfigurationMessage_init_optional_attributes(
		_DeviceConfigurationMessage);
	...
	while (offset < limit) {
		...
		case 1:
			...
			for (i = 0; i < tag; ++i) {
				offset = read_raw_byte((_DeviceConfigurationMessage->_id + i),
						_buffer, offset);
			}
			break;
		...
		case 3:
			...
			for (i = 0; i < tag; ++i) {
				offset =
					read_raw_byte((_DeviceConfigurationMessage->_sensors + i),
						_buffer, offset);
			}
			break;
		...
		case 4:
			...
			for (i = 0; i < tag; ++i) {
				offset =
					read_raw_byte((_DeviceConfigurationMessage->_commands + i),
						_buffer, offset);
			}
			break;
		}
...
int DeviceConfigurationMessage_read_delimited_from(void *_buffer,
	struct DeviceConfigurationMessage *_DeviceConfigurationMessage,
	int offset) {
	unsigned long size;

	offset = read_raw_varint32(&size, _buffer, offset);
	DeviceConfigurationMessage_read(_buffer, _DeviceConfigurationMessage,
		offset, size + offset);
