...
static void board_clocks_init(void) {
    ...
    if (REG_LOAD(CPU_IDENTITY_REG) == 0) {
        ...
        clkctr_set_pll(CLOCK_BASE, BOARD_XTI_FREQ, (BOARD_FCLK_FREQ / BOARD_XTI_FREQ) - 1);
...
void arch_init(void) {
	...
	REG32_ORIN(FPU_CPACR, (3UL << 0 * 2) | (3UL << 1 * 2)); /* set CP0, CP1 Full Access in Secure mode (enable PowerQuad) */
	REG32_ORIN(FPU_NSACR, 0xf << 20);
	REG32_ORIN(FPU_NSACR, (3UL << 0 * 2) | (3UL << 1 * 2)); /* enable CP0, CP1, CP10, CP11 Non-secure Access  */
