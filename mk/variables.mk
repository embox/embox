...
export LN     := ln -s
export MD5    := $(shell for i in md5 md5sum; do type $$i >/dev/null 2>&1 && echo $$i && exit; done; echo md5sum)
export CPIO   := $(shell for i in gcpio cpio; do type $$i >/dev/null 2>&1 && echo $$i && exit; done; echo cpio)
export AWK    := $(shell for i in gawk awk nawk mawk; do type $$i >/dev/null 2>&1 && echo $$i && exit; done; echo awk)
export TSORT  := tsort
